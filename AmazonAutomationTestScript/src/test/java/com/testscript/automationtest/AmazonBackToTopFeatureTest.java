...
public class AmazonBackToTopFeatureTest extends Amazonbaseclass {
	
	@Test(dataProvider="GetData")
	public void amazonValidateBackToTopButton(String cate, String product ) throws InterruptedException

	{
		...
		ExtentTest test=extent.createTest("Amazon End to End Test 4", "Validate Back to Top button is Present or not");
		//test.log(Status.INFO, "This Step Shows Usage of log");
		test.info("Back to Button is present on below the product page or not");
		...
		jse.executeScript("arguments[0].scrollIntoView(true)",clickcareers);
		List<WebElement> dynamicElement = Amazonpagetest.getBackToTop();
		if(dynamicElement.size() != 0)
		{
			test.pass("Back to Top Feature is Present"); 
			System.out.println("Element present");
		}
		else {
			test.fail("Back to Top  Feature is not Present");
